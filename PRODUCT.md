...
4. Provider 调用还设有独立输入/输出预算；单局与生涯使用不同上限。
5. 默认每个用户每天有 10,000 tokens 的进程内预算保护。
...
- Spring MVC、Validation、Security、OAuth2 Resource Server。
...
- 项目仓库当前未提供 LICENSE 文件，因此不能默认视为开放源代码许可项目。

## 许可证

仓库当前未提供 LICENSE 文件，也就没有授予默认的开源使用、修改或分发许可。使用与分发规则需要由项目维护者后续明确。
