...
describe("Grid Shot HUD", () => {
  ...
  it.each(["playing", "paused"])("renders real metrics while %s", () => { const stats = createEmptyGridShotStats(); Object.assign(stats, { score: 12480, accuracy: 91.4, hits: 72, shots: 79, combo: 18, maxCombo: 34, targetsPerMinute: 138 }); const html = renderToStaticMarkup(<TrainingHud stats={stats} remaining={38} personalBest={10000} fps={144} />); expect(html).toContain("12,480"); expect(html).toContain("00:38"); expect(html).toContain("91.4%"); expect(html).toContain("FPS"); expect(html).toContain("144"); });
