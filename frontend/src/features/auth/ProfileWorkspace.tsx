...
function SecurityCenter() {
  ...
  return (
    <motion.div className="security-center" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <AnimatePresence>
        {sessionAction && <ConfirmationDialog icon={sessionAction === "logout" ? LogOut : UsersRound} title={sessionAction === "logout" ? tx("退出当前设备？", "Sign out this device?") : tx("退出所有设备？", "Sign out all devices?")} description={sessionAction === "logout" ? tx("退出后将恢复访客偏好；画质和音频设置仍保留在本机，再次登录会同步该账号的控制、准星与训练偏好。", "Guest preferences return after sign-out. Display and audio stay on this device; controls, crosshair, and training preferences sync again when you sign in.") : tx("所有设备上的登录状态都会立即失效，需要重新输入账号和密码。", "Every session will end immediately and require a fresh sign-in.")} confirmLabel={sessionAction === "logout" ? tx("确认退出", "Sign out") : tx("退出所有设备", "Sign out all devices")} onCancel={() => setSessionAction(null)} onConfirm={confirmSessionAction} busy={busy !== null} />}
